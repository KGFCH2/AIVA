...
class CommandService {
  ...
  async processCommand(command) {
    ...
    if (lowerCmd.includes('debasmita') || lowerCmd.includes('debosmita') || lowerCmd.includes('who is debosmita') || lowerCmd.includes('who is debasmita')) {
      return "My creator is Debasmita Bose. She is currently reading Btech CSE 3rd yr in Adamas University. If you want to contact her, search her in the links - \nLinkedIn: https://www.linkedin.com/in/debasmita-bose2023/ \nGitHub: https://github.com/DebasmitaBose0?tab=repositories";
    }
    ...
    if (lowerCmd.includes('namaste') || lowerCmd.includes('नमस्ते')) {
      return "नमस्ते! यह वेबसाइट AIVA है, जिसे Debasmita Bose ने बनाया है। आपकी कैसे मदद कर सकती हूँ?";
    }
    // Bengali
    if (lowerCmd.includes('nomoskar') || lowerCmd.includes('নমস্কার')) {
      return "নমস্কার! এই ওয়েবসাইটটি AIVA, Debasmita Bose দ্বারা নির্মিত। আমি কিভাবে সাহায্য করতে পারি?";
    }
    // Tamil
    if (lowerCmd.includes('vanakkam') || lowerCmd.includes('வணக்கம்')) {
      return "வணக்கம்! இந்த இணையதளம் AIVA, Debasmita Bose உருவாக்கியது. நான் எப்படி உதவலாம்?";
    }
    // Telugu
    if (lowerCmd.includes('namaskaram') || lowerCmd.includes('నమస్కారం')) {
      return "నమస్కారం! ఈ వెబ్‌సైట్ AIVA, Debasmita Bose రూపొందించింది. నేను ఎలా సహాయపడగలను?";
    }
    // Marathi
    if (lowerCmd.includes('namaskar') || lowerCmd.includes('नमस्कार')) {
      return "नमस्कार! ही वेबसाइट AIVA, Debasmita Bose यांनी तयार केली आहे. मी कशी मदत करू?";
    }
    // Gujarati
    if (lowerCmd.includes('namaste') || lowerCmd.includes('નમસ્તે')) {
      return "નમસ્તે! આ વેબસાઇટ AIVA, Debasmita Bose દ્વારા બનાવવામાં આવી છે. હું કેવી રીતે મદદ કરી શકું?";
    }
    // Kannada
    if (lowerCmd.includes('namaskara') || lowerCmd.includes('ನಮಸ್ಕಾರ')) {
      return "ನಮಸ್ಕಾರ! ಈ ವೆಬ್‌ಸೈಟ್ AIVA, Debasmita Bose ರಚಿಸಿದ್ದಾರೆ. ನಾನು ಹೇಗೆ ಸಹಾಯ ಮಾಡಬಹುದು?";
    }
    // Malayalam
    if (lowerCmd.includes('namaskaram') || lowerCmd.includes('നമസ്കാരം')) {
      return "നമസ്കാരം! ഈ വെബ്സൈറ്റ് AIVA, Debasmita Bose സൃഷ്ടിച്ചത്. ഞാൻ എങ്ങനെ സഹായിക്കാം?";
    }
    ...
    if (
      ...
      return this.randomResponse([
        "This website, AIVA, was created by Debasmita Bose.",
        "Debasmita Bose built me. Give her a shout if you like!",
      ]);
    }
